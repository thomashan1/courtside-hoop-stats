...
**Adding an `EventType` case is a cross-version event.** A `Game` reaches followers as one JSON blob, so a value an older build doesn't recognise used to throw and take *the whole game* with it — `CloudKitSchema.game(from:)` returns nil and the caller skips it, leaving the game silently missing on any device that hasn't updated. `EventType` now has a lenient `init(from:)` that decodes an unknown raw value to `.unknown` (worth 0 points, never written by this build), so from v1.7 onward a future case degrades to one uncountable event instead. It cannot protect builds already shipped.
